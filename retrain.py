...
def get_random_distorted_bottlenecks(
    ...
    for unused_i in range(how_many):
        ...
        image_path = get_image_path(image_lists, label_name, image_index, image_dir,
                                    category)
        if not tf.compat.v2.io.gfile.Exists(image_path):
            tf.compat.v1.logging.fatal('File does not exist %s', image_path)
        jpeg_data = tf.compat.v2.io.gfile.FastGFile(image_path, 'rb').read()
...
def main(_):
    ...
    with tf.compat.v1.Session(graph=graph) as sess:
        ...
        tf.compat.v1.summary.scalar("loss", 0.07) 
        merged = tf.compat.v1.summary.merge_all()
        print('merged0:', type(merged))
        ...
        for i in range(1, FLAGS.how_many_training_steps):
